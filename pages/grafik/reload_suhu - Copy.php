...
$ntuk      = mysqli_query($koneksi, "SELECT celcius FROM realtimesuhu  WHERE celcius>=0 ORDER BY waktu DESC LIMIT 1");
...
$ratarata    = mysqli_query($koneksi, "SELECT  celcius, avg(celcius) as rata FROM realtimesuhu GROUP BY celcius ORDER BY rata LIMIT 1");
...
$celciusa = mysqli_query($koneksi, "SELECT celcius FROM realtimesuhu WHERE celcius>=0 ORDER BY waktu DESC LIMIT 20");
...
<table   class="table table-striped table-bordered" cellspacing="0" width="100%" id="datatables">
  ...
  $query = 'SELECT waktu,celcius from realtimesuhu ORDER BY waktu DESC LIMIT 3';
